...
def compute_loss_pip(
    ...
    loss_x = criterion_reg(
        outputs_local_x_select * masks_local_x_select,
        labels_local_x_select * masks_local_x_select,
    )
    if not masks_local_x_select.sum() == 0:
        loss_x /= masks_local_x_select.sum()
    loss_y = criterion_reg(
        outputs_local_y_select * masks_local_y_select,
        labels_local_y_select * masks_local_y_select,
    )
    if not masks_local_y_select.sum() == 0:
        loss_y /= masks_local_y_select.sum()
    loss_nb_x = criterion_reg(
        outputs_nb_x_select * masks_nb_x_select, labels_nb_x_select * masks_nb_x_select
    )
    if not masks_nb_x_select.sum() == 0:
        loss_nb_x /= masks_nb_x_select.sum()
    loss_nb_y = criterion_reg(
        outputs_nb_y_select * masks_nb_y_select, labels_nb_y_select * masks_nb_y_select
    )
        ...
        for i, data in enumerate(train_loader):
            if det_head == "pip":
                ...
                loss = (
                    ...
                    + reg_loss_weight * loss_x
                    + reg_loss_weight * loss_y
                    + reg_loss_weight * loss_nb_x
            ...
            if i % 10 == 0:
                if det_head == "pip":
                    print(
                        "[Epoch {:d}/{:d}, Batch {:d}/{:d}] <Total loss: {:.6f}> <map loss: {:.6f}> <x loss: {:.6f}> <y loss: {:.6f}> <nbx loss: {:.6f}> <nby loss: {:.6f}>".format(
                            epoch,
                            num_epochs - 1,
                            i,
                            len(train_loader) - 1,
                            loss.item(),
                            cls_loss_weight * loss_map.item(),
                            reg_loss_weight * loss_x.item(),
                            reg_loss_weight * loss_y.item(),
                            reg_loss_weight * loss_nb_x.item(),
...
def forward_pip(net, inputs, preprocess, input_size, net_stride, num_nb):
    ...
    with torch.no_grad():
        ...
        tmp_y = (max_ids // tmp_width).view(-1, 1).float() + outputs_y_select.view(
            -1, 1
        )
    ...
    return (
        ...
        tmp_nb_x,
        tmp_nb_y,
        [outputs_cls1, outputs_cls2, outputs_cls3],
        max_cls,
    )
