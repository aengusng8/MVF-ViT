...
def process_300w(root_folder, folder_name, image_name, label_name, target_size):
    ...
    with open(label_path, "r") as ff:
        ...
        anno = [
            [(x - bbox_xmin) / bbox_width, (y - bbox_ymin) / bbox_height]
            for x, y in anno
        ]
...
def process_wflw(anno, target_size):
    ...
    height = bbox_ymax - bbox_ymin
    image_crop = image[
        int(bbox_ymin) : int(bbox_ymax), int(bbox_xmin) : int(bbox_xmax), :
    ]
    image_crop = cv2.resize(image_crop, (target_size, target_size))
...
def convert_wflw(root_folder, data_name):
    ...
    for anno in annos:
        ...
        annos_new[-1].append(
            str((float(anno[34 * 2 + 1]) + float(anno[41 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[35 * 2])
        annos_new[-1].append(
            str((float(anno[35 * 2 + 1]) + float(anno[40 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[36 * 2])
        annos_new[-1].append(
            str((float(anno[36 * 2 + 1]) + float(anno[39 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[37 * 2])
        annos_new[-1].append(
            str((float(anno[37 * 2 + 1]) + float(anno[38 * 2 + 1])) / 2)
        )
        # right eyebrow
        annos_new[-1].append(anno[42 * 2])
        annos_new[-1].append(
            str((float(anno[42 * 2 + 1]) + float(anno[50 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[43 * 2])
        annos_new[-1].append(
            str((float(anno[43 * 2 + 1]) + float(anno[49 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[44 * 2])
        annos_new[-1].append(
            str((float(anno[44 * 2 + 1]) + float(anno[48 * 2 + 1])) / 2)
        )
        annos_new[-1].append(anno[45 * 2])
        annos_new[-1].append(
            str((float(anno[45 * 2 + 1]) + float(anno[47 * 2 + 1])) / 2)
        )
        ...
        annos_new[-1].append(
            str(0.666 * float(anno[61 * 2]) + 0.333 * float(anno[62 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[61 * 2 + 1]) + 0.333 * float(anno[62 * 2 + 1]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[63 * 2]) + 0.333 * float(anno[62 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[63 * 2 + 1]) + 0.333 * float(anno[62 * 2 + 1]))
        )
        annos_new[-1].append(anno[64 * 2])
        annos_new[-1].append(anno[64 * 2 + 1])
        annos_new[-1].append(
            str(0.666 * float(anno[65 * 2]) + 0.333 * float(anno[66 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[65 * 2 + 1]) + 0.333 * float(anno[66 * 2 + 1]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[67 * 2]) + 0.333 * float(anno[66 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[67 * 2 + 1]) + 0.333 * float(anno[66 * 2 + 1]))
        )
        # right eye
        annos_new[-1].append(anno[68 * 2])
        annos_new[-1].append(anno[68 * 2 + 1])
        annos_new[-1].append(
            str(0.666 * float(anno[69 * 2]) + 0.333 * float(anno[70 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[69 * 2 + 1]) + 0.333 * float(anno[70 * 2 + 1]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[71 * 2]) + 0.333 * float(anno[70 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[71 * 2 + 1]) + 0.333 * float(anno[70 * 2 + 1]))
        )
        annos_new[-1].append(anno[72 * 2])
        annos_new[-1].append(anno[72 * 2 + 1])
        annos_new[-1].append(
            str(0.666 * float(anno[73 * 2]) + 0.333 * float(anno[74 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[73 * 2 + 1]) + 0.333 * float(anno[74 * 2 + 1]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[75 * 2]) + 0.333 * float(anno[74 * 2]))
        )
        annos_new[-1].append(
            str(0.666 * float(anno[75 * 2 + 1]) + 0.333 * float(anno[74 * 2 + 1]))
        )
        # mouth
...
def gen_data(root_folder, data_name, target_size):
    ...
    if data_name == "CELEBA":
        ...
        with open(os.path.join(root_folder, data_name, "train.txt"), "w") as f:
            for bbox in bboxes:
                ...
                cv2.imwrite(
                    os.path.join(root_folder, data_name, "images_train", image_name),
                    image_crop,
                )
    ################################################################################################################
        ...
        os.system(
            "cp ../data/CELEBA/train.txt ../data/data_300W_CELEBA/train_CELEBA.txt"
        )
        ...
        for folder_train in folders_train:
            all_files = sorted(
                os.listdir(os.path.join(root_folder, "data_300W", folder_train))
            )
            ...
            for image_name, label_name in zip(image_files, label_files):
                print(image_name)
                image_crop, anno = process_300w(
                    os.path.join(root_folder, "data_300W"),
                    folder_train,
                    image_name,
                    label_name,
                    target_size,
                ...
                cv2.imwrite(
                    os.path.join(
                        root_folder, data_name, "images_train", image_crop_name
                    ),
                    image_crop,
                )
        ...
        for folder_test in folders_test:
            all_files = sorted(
                os.listdir(os.path.join(root_folder, "data_300W", folder_test))
            )
            image_files = [x for x in all_files if ".pts" not in x]
            label_files = [x for x in all_files if ".pts" in x]
            assert len(image_files) == len(label_files)
            for image_name, label_name in zip(image_files, label_files):
                ...
                cv2.imwrite(
                    os.path.join(
                        root_folder, data_name, "images_test_300W", image_crop_name
                    ),
                    image_crop,
                )
        ...
        # test for COFW_68
        test_mat = hdf5storage.loadmat(
            os.path.join("../data/COFW", "COFW_test_color.mat")
        )
        images = test_mat["IsT"]

        bboxes_mat = hdf5storage.loadmat(
            os.path.join("../data/data_300W_CELEBA", "cofw68_test_bboxes.mat")
        )
        ...
        with open("../data/data_300W_CELEBA/test_COFW.txt", "w") as f:
            for i in range(image_num):
                ...
                bbox = bboxes[i, :]
                anno_mat = hdf5storage.loadmat(
                    os.path.join(
                        "../data/data_300W_CELEBA/cofw68_test_annotations",
                        str(i + 1) + "_points.mat",
                    )
                )
                ...
                cv2.imwrite(
                    os.path.join(
                        "../data/data_300W_CELEBA/images_test_COFW", image_crop_name
                    ),
                    image_crop,
                )
                f.write(image_crop_name + " ")
        ...
        count = 1
        with open(
            os.path.join(root_folder, "data_300W_CELEBA", "test_WFLW_98.txt"), "w"
        ) as f:
            for anno_test in annos_test:
                ...
                names_mapping[anno_test[0] + "_" + anno_test[-1]] = [
                    image_crop_name,
                    anno,
                ]
                cv2.imwrite(
                    os.path.join(
                        root_folder, data_name, "images_test_WFLW", image_crop_name
                    ),
                    image_crop,
                )
                f.write(image_crop_name + " ")
    ...
    ################################################################################################################
        ...
        bboxes_mat = hdf5storage.loadmat(
            os.path.join("../data/data_300W_COFW_WFLW", "cofw68_test_bboxes.mat")
        )
        bboxes = bboxes_mat["bboxes"]
        image_num = images.shape[0]
        with open("../data/data_300W_COFW_WFLW/test_COFW.txt", "w") as f:
            for i in range(image_num):
                ...
                anno_mat = hdf5storage.loadmat(
                    os.path.join(
                        "../data/data_300W_COFW_WFLW/cofw68_test_annotations",
                        str(i + 1) + "_points.mat",
                    )
                )
                anno = anno_mat["Points"]
                ...
                cv2.imwrite(
                    os.path.join(
                        "../data/data_300W_COFW_WFLW/images_test_COFW", image_crop_name
                    ),
                    image_crop,
                )
                f.write(image_crop_name + " ")
        ...
        count = 1
        with open(
            os.path.join(root_folder, "data_300W_COFW_WFLW", "test_WFLW_98.txt"), "w"
        ) as f:
            for anno_test in annos_test:
                image_crop, anno = process_wflw(anno_test, target_size)
                pad_num = 4 - len(str(count))
