...
def random_translate(image, target):
    if random.random() > 0.5:
        ...
        target_translate[:, 0] -= 1.0 * c / image_width
        target_translate[:, 1] -= 1.0 * f / image_height
...
def random_occlusion(image):
    if random.random() > 0.5:
        ...
        image_np[
            occ_ymin : occ_ymin + occ_height, occ_xmin : occ_xmin + occ_width, 0
        ] = int(random.random() * 255)
        image_np[
            occ_ymin : occ_ymin + occ_height, occ_xmin : occ_xmin + occ_width, 1
        ] = int(random.random() * 255)
        image_np[
            occ_ymin : occ_ymin + occ_height, occ_xmin : occ_xmin + occ_width, 2
        ] = int(random.random() * 255)
...
class ImageFolder_pip(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_map2 = np.zeros(
            (
                self.num_lms,
                int(self.input_size / self.net_stride / 2),
                int(self.input_size / self.net_stride / 2),
            )
        )
        target_map3 = np.zeros(
            (
                self.num_lms,
                int(self.input_size / self.net_stride / 4),
                int(self.input_size / self.net_stride / 4),
            )
        )
        ...
        target_nb_x = np.zeros(
            (
                self.num_nb * self.num_lms,
                int(self.input_size / self.net_stride),
                int(self.input_size / self.net_stride),
            )
        )
        target_nb_y = np.zeros(
            (
                self.num_nb * self.num_lms,
                int(self.input_size / self.net_stride),
                int(self.input_size / self.net_stride),
