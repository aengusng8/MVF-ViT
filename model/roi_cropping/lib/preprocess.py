...
def process_aflw(root_folder, image_name, bbox, anno, target_size):
    ...
    anno = np.concatenate(
        [anno_x.reshape(-1, 1), anno_y.reshape(-1, 1)], axis=1
    ).flatten()
...
def gen_data(root_folder, data_name, target_size):
    ...
    if data_name == "data_300W":
        ...
        for folder_train in folders_train:
            all_files = sorted(
                os.listdir(os.path.join(root_folder, data_name, folder_train))
            )
        ...
        for folder_test in folders_test:
            all_files = sorted(
                os.listdir(os.path.join(root_folder, data_name, folder_test))
            )
            image_files = [x for x in all_files if ".pts" not in x]
            label_files = [x for x in all_files if ".pts" in x]
            assert len(image_files) == len(label_files)
            for image_name, label_name in zip(image_files, label_files):
                ...
                cv2.imwrite(
                    os.path.join(
                        root_folder, data_name, "images_test", image_crop_name
                    ),
                    image_crop,
                )
    ...
    ################################################################################################################
        ...
        with open(os.path.join(root_folder, "COFW", "train.txt"), "w") as f:
            for i in range(images.shape[0]):
                ...
                cv2.imwrite(
                    os.path.join(root_folder, "COFW", "images_train", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
                for x, y in anno:
        ...
        with open(os.path.join(root_folder, "COFW", "test.txt"), "w") as f:
            for i in range(images.shape[0]):
                ...
                print(image_crop_name)
                cv2.imwrite(
                    os.path.join(root_folder, "COFW", "images_test", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
                for x, y in anno:
    ...
    ################################################################################################################
        ...
        with open(os.path.join(root_folder, "WFLW", "train.txt"), "w") as f:
            for anno_train in annos_train:
                ...
                print(image_crop_name)
                cv2.imwrite(
                    os.path.join(root_folder, "WFLW", "images_train", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
                for x, y in anno:
        ...
        with open(os.path.join(root_folder, "WFLW", "test.txt"), "w") as f:
            for anno_test in annos_test:
                ...
                cv2.imwrite(
                    os.path.join(root_folder, "WFLW", "images_test", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
        ...
        with open(
            os.path.join(
                root_folder, "WFLW", "WFLW_annotations", "list_98pt_test", test_mu_file
            ),
            "r",
        ) as f:
    ...
    ################################################################################################################
        ...
        with open(os.path.join(root_folder, "AFLW", "train.txt"), "w") as f:
            for index in train_indices:
                ...
                bbox = bboxes[index - 1]
                anno = annos[index - 1]
                image_crop, anno = process_aflw(
                    root_folder, image_name, bbox, anno, target_size
                )
                pad_num = 5 - len(str(index))
                image_crop_name = "aflw_train_" + "0" * pad_num + str(index) + ".jpg"
                print(image_crop_name)
                cv2.imwrite(
                    os.path.join(root_folder, "AFLW", "images_train", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
                for x, y in anno:
        ...
        with open(os.path.join(root_folder, "AFLW", "test.txt"), "w") as f:
            for index in test_indices:
                ...
                print(image_crop_name)
                cv2.imwrite(
                    os.path.join(root_folder, "AFLW", "images_test", image_crop_name),
                    image_crop,
                )
                f.write(image_crop_name + " ")
                for x, y in anno:
