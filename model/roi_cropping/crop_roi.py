...
if not len(sys.argv) == 2:
    print('Format:')
    print('python lib/roi_crop.py config_file image_file')
    exit(0)
...
def roi_crop(image_file, net=net, preprocess=preprocess, input_size=cfg.input_size, net_stride=cfg.net_stride, num_nb=cfg.num_nb, use_gpu=cfg.use_gpu, device=device):
    ...
    for i in range(len(detections)):
        ...
        for i in range(cfg.num_lms):
            ...
            cv2.putText(clone, str(i), (int(x_pred)+det_xmin, int(y_pred)+det_ymin), cv2.FONT_HERSHEY_SIMPLEX,
                        0.7, (0, 0, 255), 1)
        ...
        for (name, (i, j)) in FACIAL_LANDMARKS_IDXS.items():
          ...
          roi = imutils.resize(roi, width=224, height=224, inter=cv2.INTER_CUBIC)
          print(roi.shape)
          rois.append(roi)
