...
class MPPLoss(nn.Module):
    def __init__(
        self, patch_size, channels, output_channel_bits, max_pixel_val, mean, std
    ):
        super().__init__()
    ...
    def forward(self, predicted_patches, target, mask):
        ...
        avg_target = reduce(
            target, "b c (h p1) (w p2) -> b (h w) c", "mean", p1=p, p2=p
        ).contiguous()
...
class MPP(nn.Module):
    def __init__(
        ...
        self.loss = MPPLoss(
            patch_size, channels, output_channel_bits, max_pixel_val, mean, std
        )
    ...
    def forward(self, input, **kwargs):
        ...
        if self.random_patch_prob > 0:
            ...
            random_patch_prob = prob_mask_like(input, random_patch_sampling_prob).to(
                mask.device
            )

            bool_random_patch_prob = mask * (random_patch_prob == True)
            random_patches = torch.randint(
                0, input.shape[1], (input.shape[0], input.shape[1]), device=input.device
            )
            randomized_input = masked_input[
                torch.arange(masked_input.shape[0]).unsqueeze(-1), random_patches
            ]
